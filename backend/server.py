...
async def generate_guide(req: GenerationRequest, request: Request):
    ...
    try:
        ...
        pdf.rect(0, pdf.h * 0.38, pdf.w, pdf.h * 0.26, "F")
        pdf.set_y(pdf.h * 0.42)
        pdf.set_font(theme["font_family"], "B", 34)
        pdf.set_text_color(*theme["text"])
        pdf.cell(0, 14, req.subject, align="C", ln=True)
        pdf.set_font(theme["font_family"], "", 14)
        pdf.cell(0, 8, f"Prepared for: {req.author}", align="C", ln=True)
